...
def _load_vertex_credentials():
    try:
        if "vertex_ai" not in st.secrets:
            st.error("vertex_ai section missing in secrets")
            return False
        ...
        st.success("Vertex initialized successfully")
        return True

    except Exception as e:
        st.error("Vertex initialization failed:")
        st.error(str(e))
        return False
